...
class Jobs {
	...
	waitForHealthCheck(callback) {
		...
		var url = (this.config.get('secure') ? 'https:' : 'http:') + '//' + this.socket.host + ':' + this.socket.port + '/health';
	...
	uploadJobFiles(job, callback) {
		...
		var url = (this.config.get('secure') ? 'https:' : 'http:') + '//' + this.socket.host + ':' + this.socket.port + '/api/app/upload_job_file';
	...
	updateJobFinal(job) {
		...
		this.waitForHealthCheck( function() {
			...
			var url = (self.config.get('secure') ? 'https:' : 'http:') + '//' + self.socket.host + ':' + self.socket.port + '/api/app/finish_job';
