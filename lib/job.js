...
class Jobs {
	...
	launchJob(job, details, sec) {
		...
		if (!this.platform.windows) {
			...
			// make sure the job can write to its own temp dir and input files
			try {
				fs.chownSync( job.cwd, child_opts.uid, child_opts.gid );
				fs.readdirSync( job.cwd ).forEach( function(filename) {
					fs.chownSync( Path.join(job.cwd, filename), child_opts.uid, child_opts.gid );
				} );
			}
			catch (err) {
				job.pid = 0;
