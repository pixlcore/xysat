...
class Jobs {
	...
	handleChildResponse(job, worker, data) {
		...
		// merge in data
		if (data.opsrocket) {
			// new api: provide `opsrocket` key and everything else gets imported
			Tools.mergeHashInto( job, Tools.copyHashRemoveKeys(data, { opsrocket:1 }) );
			found = true;
		}
		else {
