...
class Jobs {
	...
	launchJob(job, details, sec) {
		...
		if (job.workflowData) {
			for (var key in job.workflowData) {
				child_opts.env[ 'workflow_data_' + key.replace(/\W+/g, '_') ] = niceString(job.workflowData[key]);
			}
		}
		
		// add server data if applicable, and with special server_data_ key prefix
		if (job.serverData) {
			for (var key in job.serverData) {
				child_opts.env[ 'server_data_' + key.replace(/\W+/g, '_') ] = niceString(job.serverData[key]);
			}
		}
		
		// add top-level input data params if applicable, and with special data_ key prefix
		if (job.input && job.input.data) {
			for (var key in job.input.data) {
				child_opts.env[ 'data_' + key.replace(/\W+/g, '_') ] = niceString(job.input.data[key]);
			}
		}
