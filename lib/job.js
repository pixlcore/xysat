...
// Copyright (c) 2020 - 2025 Joseph Huckaby
				...
				async.eachSeries( job.input.files,
					function(file, callback) {
						...
						self.request.get( url, opts, function(err, resp, data, perf) {
							if (err) {
								return callback( new Error("Failed to download job file: " + filename + ": " + (err.message || err)) );
							}
							callback();
						} ); // request.get
					},
	...
	uploadJobFiles(job, callback) {
		...
		async.eachSeries( job.files,
			function(file, callback) {
				...
				self.appendMetaLog(job, "Uploading file: " + file.path + " (" + filename + ")");
