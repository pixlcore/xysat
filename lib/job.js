...
class Jobs {
		...
		async.eachSeries( job.files,
			function(file, callback) {
				...
				var opts = Tools.mergeHashes( self.config.get('socket_opts') || {}, {
					...
					retries: self.config.getPath('socket_opts.retries') || 8, 
					retryDelay: self.config.getPath('socket_opts.retryDelay') || 50
				});
	...
	updateJob(job, final) {
		...
		if (!this.socket || !this.socket.connected || !this.socket.auth) {
			if (final && !this.server.shut) {
				self.logJob(9, "No connection, will retry final job update: " + job.id);
				setTimeout( function() { self.updateJob(job, final); }, 1000 );
			}
			return;
		}
