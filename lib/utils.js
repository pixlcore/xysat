...
class Utils {
	...
	cleanEnv() {
		...
		for (var key in env) {
			if (key.match(/^(XYOPS_|SATELLITE_)/)) delete env[key];
		}
	...
	upgradeSatellite() {
		...
		if (this.platform.windows) {
			...
			var log_file = Path.join( process.cwd(), 'logs', 'Maint.log' );
		...
		var fd = 0;
		if (!this.platform.windows) {
			fd = fs.openSync( Path.join( this.config.get('log_dir'), 'Maint.log' ), 'a' );
