...
class Utils {
	...
	getNetworkInterfacesFast(callback) {
		// get detailed network interface info without launching any subprocesses
		// (linux only -- combines Node's address data, /proc counters and /sys metadata)
		var self = this;
		var now = Date.now();
		var result = {
			interfaces: {},
		...
		// Node provides all assigned addresses without any shell commands.
		// Keep this guarded, as libuv can very occasionally fail this call.
		var native_ifaces = {};
		try { native_ifaces = os.networkInterfaces() || {}; }
		catch (err) {
			this.logError('monitor', "Failed to read network interfaces: " + err);
			return callback(result);
		}
		if (!this.interfaceState) this.interfaceState = {};
		
		// Refresh the shared snapshot used by the lightweight monitor as well.
		this.interfaces = native_ifaces;
		if (!native_ifaces[this.defaultInterfaceName]) {
			this.defaultInterfaceName = Tools.firstKey(native_ifaces);
		}
		
		Object.keys(native_ifaces).forEach( function(iface_name) {
			var addresses = native_ifaces[iface_name] || [];
			var iface = result.interfaces[iface_name] = {
				iface: iface_name,
				ifaceName: iface_name,
				default: (iface_name == self.defaultInterfaceName),
				ip4: '',
				ip4subnet: '',
				ip6: '',
				ip6subnet: '',
			...
			addresses.forEach( function(address) {
				...
				iface.internal = iface.internal || !!address.internal;
				if (!iface.mac && address.mac) iface.mac = address.mac;
				
				// Prefer routable addresses over IPv4/IPv6 link-local addresses.
				if ((family == 'IPv4') || (family == 4)) {
					if (!iface.ip4 || (iface.ip4.match(/^169\.254\./) && !address.address.match(/^169\.254\./))) {
						iface.ip4 = address.address;
						iface.ip4subnet = address.netmask;
					}
				}
				else if ((family == 'IPv6') || (family == 6)) {
					if (!iface.ip6 || (iface.ip6.match(/^fe80:/i) && !address.address.match(/^fe80:/i))) {
						iface.ip6 = address.address;
						iface.ip6subnet = address.netmask;
					}
				}
			} );
			
			// Loopback devices are represented as virtual in the xyOps UI.
			if (iface.internal) iface.type = 'virtual';
		} );
		
		// Read all interface counters at once.  Unlike per-interface shell commands,
		// this remains cheap even on hosts with many bridges and virtual adapters.
		fs.readFile( '/proc/net/dev', 'utf8', function(err, data) {
			var next_state = {};
			
			if (!err) data.trim().split(/\n/).forEach( function(line) {
				var colon_idx = line.indexOf(':');
				if (colon_idx < 0) return;
				
				var iface_name = line.substring(0, colon_idx).trim();
				var iface = result.interfaces[iface_name];
				if (!iface) return;
				
				var cols = line.substring(colon_idx + 1).trim().split(/\s+/);
				if (cols.length < 16) return;
				
				iface.rx_bytes = parseInt(cols[0], 10) || 0;
				iface.rx_errors = parseInt(cols[2], 10) || 0;
				iface.rx_dropped = parseInt(cols[3], 10) || 0;
				iface.tx_bytes = parseInt(cols[8], 10) || 0;
				iface.tx_errors = parseInt(cols[10], 10) || 0;
				iface.tx_dropped = parseInt(cols[11], 10) || 0;
				
				var prev = self.interfaceState[iface_name];
				if (prev && prev.ms && (now > prev.ms)) {
					iface.ms = now - prev.ms;
					iface.rx_sec = Math.max( 0, iface.rx_bytes - prev.rx_bytes ) / (iface.ms / 1000);
					iface.tx_sec = Math.max( 0, iface.tx_bytes - prev.tx_bytes ) / (iface.ms / 1000);
				}
				
				next_state[iface_name] = {
					ms: now,
			...
			// Replace the cache so removed interfaces do not accumulate forever.
			// Retain the last good sample if /proc was temporarily unavailable.
			if (!err) self.interfaceState = next_state;
			
			// Add up the same external-interface totals previously supplied by SI.
			Object.keys(result.interfaces).forEach( function(iface_name) {
				var iface = result.interfaces[iface_name];
				if (iface.internal) return;
				
				result.stats.ifaces.push(iface_name);
				result.stats.rx_bytes += iface.rx_bytes;
				result.stats.rx_dropped += iface.rx_dropped;
				result.stats.rx_errors += iface.rx_errors;
				result.stats.rx_sec += iface.rx_sec || 0;
				result.stats.tx_bytes += iface.tx_bytes;
				result.stats.tx_dropped += iface.tx_dropped;
				result.stats.tx_errors += iface.tx_errors;
				result.stats.tx_sec += iface.tx_sec || 0;
			} );
			
			// Enrich the address/counter data from small asynchronous sysfs reads.
			// Limit concurrency so a host with hundreds of adapters stays well behaved.
			async.eachLimit( Object.keys(result.interfaces), 16,
				function(iface_name, done) {
					var iface = result.interfaces[iface_name];
					if (!iface_name.match(/^[\w.\-:@]+$/)) return done();
					
					var sys_path = Path.join('/sys/class/net', iface_name);
					var virtual_path = Path.join('/sys/devices/virtual/net', iface_name);
					var read_value = function(path, next) {
					...
					async.parallel({
						operstate: function(next) { read_value( Path.join(sys_path, 'operstate'), next ); },
						duplex: function(next) { read_value( Path.join(sys_path, 'duplex'), next ); },
						mtu: function(next) { read_value( Path.join(sys_path, 'mtu'), next ); },
						speed: function(next) { read_value( Path.join(sys_path, 'speed'), next ); },
						carrierChanges: function(next) { read_value( Path.join(sys_path, 'carrier_changes'), next ); },
						virtual: function(next) { fs.stat( virtual_path, function(err) { next(null, !err); } ); },
						wireless: function(next) { fs.stat( Path.join(sys_path, 'wireless'), function(err) { next(null, !err); } ); }
					},
					function(err, values) {
						// Individual empty values are expected for unsupported fields.
						values = values || {};
						if (values.operstate) iface.operstate = values.operstate.trim();
						if (values.duplex) iface.duplex = values.duplex.trim();
						if (values.mtu) iface.mtu = parseInt(values.mtu, 10) || 0;
						if (values.carrierChanges) iface.carrierChanges = parseInt(values.carrierChanges, 10) || 0;
						
						var speed = parseInt(values.speed, 10);
						iface.speed = ((speed >= 0) && isFinite(speed)) ? speed : null;
						iface.virtual = !iface.internal && !!values.virtual;
						
						if (iface.internal || values.virtual) iface.type = 'virtual';
						else if (values.wireless) iface.type = 'wireless';
						else iface.type = 'wired';
						done();
					}
					);
				},
	...
	upgradeSatellite() {
		...
		// make sure only one upgrade runs at a time
		if (this.upgradeInProgress) {
			this.logError('upgrade', "An upgrade is already in progress. Aborting upgrade request.");
			return;
		}
		...
		if (this.platform.windows) {
			...
			cmd = `schtasks /Create /TN "${task}" /SC ONSTART /RU SYSTEM /RL HIGHEST /TR "${tr}"`;
		...
		var timer = setTimeout( function() {
			// release upgrade lock after timeout, so failed upgrades can be retried
			delete self.upgradeInProgress;
