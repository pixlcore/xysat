...
class Utils {
	...
	archiveLogs() {
		...
		fs.stat( this.logger.path, function(err, stats) {
			if (err || !stats.size) return; // log is missing or 0 bytes
			
			self.logDebug(5, "Beginning daily log archive to: " + arch_path);
			
			self.logger.archive( '', arch_path, now - 1080, function(err) {
				if (err) self.logError('archive', "Failed to archive logs: " + err);
				else self.logDebug(5, "Log archive complete");
				...
				function(err, files) {
					...
					async.eachSeries(files,
						function(file, callback) {
							self.logDebug(6, "Deleting old log archive: " + file);
						...
						function() {
							self.logDebug(5, "Log archive deletion complete");
						}
					);
				});
			} ); // archive
