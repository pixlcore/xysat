...
class Utils {
	...
	getCPUFast(skey, callback) {
		...
		if (this.platform.linux) {
			...
			fs.readFile( '/proc/loadavg', 'utf8', function(err, raw_load) {
				// use /proc/loadavg on linux, as it's more accurate inside containers vs. Node's os.loadavg()
				if (raw_load && raw_load.match(/^\s*([\d\.]+)/)) info.avgLoad = parseFloat( RegExp.$1 );
