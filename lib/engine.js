...
class Satellite extends Component {
	...
	startup(callback) {
		...
		try {
			// Keep shared temp directories world-writable, but prevent users from
			// deleting or renaming entries owned by other users (as with /tmp).
			fs.chmodSync( this.config.get('temp_dir'), 0o1777 );
			fs.chmodSync( Path.join( this.config.get('temp_dir'), 'plugins' ), 0o1777 );
			fs.chmodSync( Path.join( this.config.get('temp_dir'), 'jobs' ), 0o1777 );
		}
		catch (e) {;}
