	...
	runMonitors(opts, callback) {
		...
		async.series([
			...
			function(callback) {
				...
				if (!self.dockerBin || !self.config.get('docker_enabled')) return process.nextTick( callback );
				...
				cp.execFile( self.dockerBin, ['stats', '--no-stream', '--format', 'json'], { timeout: 10000 }, function(err, stdout, stderr) {
					...
					if (err) self.logMonitor(9, "Docker stats error: " + err);
						...
						self.logMonitor(9, "Docker stats response", stdout);
						info.data.containers = [];
						stdout.trim().split(/\n/).forEach( function(line) {
							try { info.data.containers.push( JSON.parse(line) ); } catch (e) {;}
						} );
					}
					callback();
				}); // cp.execFile
