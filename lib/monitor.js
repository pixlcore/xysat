		...
		async.series([
			...
			function(callback) {
				...
				if (self.platform.linux) {
					self.logMonitor(9, "Calling getNetworkInterfacesFast...");
					perf.begin('getNetworkInterfacesFast');
					return self.getNetworkInterfacesFast( function(data) {
						perf.end('getNetworkInterfacesFast');
						info.data.interfaces = data.interfaces;
						info.data.stats.network = data.stats;
						self.logMonitor(9, "getNetworkInterfacesFast response", data);
						callback();
					} );
				}
				...
				// allow systeminformation to reuse its cache when the OS interface snapshot is unchanged
			...
			function(callback) {
				// network stats
				if (self.platform.linux) return callback(); // already collected above
