// Orchestra Satellite - Monitor Layer
...
class Monitor {
	...
	runMonitors(opts) {
		...
		async.series([
			function(callback) {
				// sleep for N seconds based on hash of hostname, scale up with numServers, max of 30s
				// this is to avoid multiple servers from submitting metrics at the same instant
				var max_sleep_ms = opts.max_sleep_ms || (Tools.clamp(self.numServers, 1, 1000) * 30);
				var sleep_ms = 0 + (self.hostID % (max_sleep_ms || 1));
				perf.begin('sleep');
