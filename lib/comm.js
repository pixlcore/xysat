...
class Communication {
	...
	checkStartupLogFiles() {
		...
		if (fs.existsSync(bkgnd_log_file)) {
			...
			this.socket.send('notice', { description: `Upgrade completed on worker server.`, details });
		...
		if (fs.existsSync(crash_log_file)) {
			...
			var details = "**Log Contents:**\n\n```\n" + contents + "\n```\n";
			this.socket.send('critical', { description: `Crash log was found on worker server.`, details });
