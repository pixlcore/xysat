...
class Communication {
	...
	handleSocketMessage(json) {
		...
		switch (cmd) {
			...
			case 'retry':
				...
				this.logDebug(5, "Master is not ready: will reconnect");
				socket.close();
			break;
			
			case 'launch_job':
				// prep and launch job
				this.prepLaunchJob(data.job, data.sec || {});
			break;
	...
	upgradeSatellite() {
		...
		this.logDebug(1, "Received upgrade request from master");
		
		// sanity
		if (this.debug || this.foreground) {
			this.logError('upgrade', "Cannot self-upgrade in debug or foreground mode.");
			return;
		}
		
		// if jobs are active, wait until they complete
		if (Tools.firstKey(this.activeJobs)) {
			this.logDebug(3, "Jobs still active, will wait until they complete");
		...
		if (this.platform.windows) {
			cmd = `powershell -NoProfile -ExecutionPolicy Bypass -Command "IEX (Invoke-WebRequest -UseBasicParsing -Uri '${url}&os=windows').Content"`;
		}
		...
		this.logDebug(3, "Executing self-upgrade command: " + cmd);
