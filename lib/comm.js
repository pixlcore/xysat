...
class Communication {
	...
	socketConnect() {
		...
		this.logDebug(5, "WebSocket Connect: " + url);
		...
		socket.ws.onclose = function (event) {
			...
			self.reconnectTimer = setTimeout( function() { self.socketConnect(); }, 1000 );
			self.socket = null;
			
			self.appendMetaLogAllJobs("Lost connection to master server");
		};
	}
	...
	handleSocketMessage(json) {
		...
		switch (cmd) {
			...
			case 'hello':
				...
				socket.send('join', {
					token: Tools.digestHex( data.nonce + this.config.get('secret_key'), 'sha256' )
				});
			break;
